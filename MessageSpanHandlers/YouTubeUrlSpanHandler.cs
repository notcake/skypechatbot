...
namespace ChatBot.MessageSpanHandlers
{
    ...
    {
        private readonly string[] YouTubeUrls =
        {
        ...
        {
            foreach (var youTubeUrlPattern in YouTubeUrls)
            {
        ...
        public void HandleSpan(MessageSink messageSink, MessageActionSpan actionSpan)
        {
            ...
            var videoInfo = new VideoInfo(videoId);
            ...
            // Music lookup
            var match = new Regex("([^\\-]+?)\\-([^\\(\\){\\[]+)").Match(videoInfo.Title);
            if (match.Success)
            {
                var recordingSearch = new RecordingSearch(match.Groups[2].ToString().Trim());
                recordingSearch.Artist = match.Groups[1].ToString().Trim();

                var normalizedSongName = new Regex("[^ a-zA-Z0-9]").Replace(recordingSearch.SearchString, "").ToLower();
                var normalizedArtistName = new Regex("[^ a-zA-Z0-9]").Replace(recordingSearch.Artist, "").ToLower();

                string artistName = null;
                string albumName = null;
                string releaseType = null;
                var releaseDate = DateTime.Now;
                var searchResults = recordingSearch.GetResults();
                foreach (var result in searchResults)
                {
                    var resultTitle = result.Title;
                    var resultArtistName = result.Artist;
                    ...
                    resultArtistName =
                        new Regex("[^ a-zA-Z0-9]").Replace(resultArtistName.Normalize(NormalizationForm.FormD), "");
                        ...
                        resultArtistName.ToLower().Contains(normalizedArtistName))
                    {
                        ...
                        {
                            if (release.Type != "Single" && release.Type != "Album")
                            {
                                continue;
                            }
                            if (release.Date > releaseDate)
                            {
                                continue;
                            }
                            if (!release.HasDate)
                ...
                if (albumName != null)
                {
                    ...
                    if (releaseType == "Album")
                    {
                        ...
                        if (!page.Exists)
                        {
                            page = new Page(albumName + " (album)");
                        }
                        if (!page.Exists)
                        {
                            page = new Page(albumName);
                        }
                        if (!page.Exists)
                        {
                            page = new Page(albumName + " (UK series)");
                        }
                    }
            ...
            var videoInfoMessage = "YouTube: " + videoInfo.Title + " [" + formattedDuration + "]";

            var blockedCountries = new List<string>();
            ...
            {
                videoInfoMessage += "\n        Blocked in ";
                for (var i = 0; i < blockedCountries.Count - 1; i++)
                {
                    if (i > 0)
                    {
                        videoInfoMessage += ", ";
